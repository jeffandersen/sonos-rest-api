...
/**
 * Find sonos player by criteria
 *
 * @public
 * @param {object} opts - uuid or name
...
/**
 * Clear the queue for a particular player
 *
 * @public
 * @param {object} where - Name or uuid property to look for
 * @param {function} cb - Callback function
 */
Players.prototype.clearQueue = function(where, cb) {
  var self = this;
  ...
  self._find(where, function(err, player) {
    ...
    player.removeAllTracksFromQueue(function(success) {
      if (!success) {
        var err = new Error('An error occurred while clearing the queue');
        err.type = 'failed';
        return cb(err);
      }
...
Players.prototype.queue = function(where, opts, cb) {
  ...
  self._find(where, function(err, player) {
    ...
    player.getQueue(offset, limit, function(success, queueData) {
      ...
      self.sonos._findAlbumArt(queueData.items, offset, function(err, tracks) {
        if (err) {
          return cb(err);
        }
