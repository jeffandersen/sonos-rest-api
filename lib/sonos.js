...
// Primitive cache for albumart URLs rather than introducing keystore right now
var ALBUM_ART_CACHE = {};
...
Sonos.prototype._findAlbumArt = function(track, cb) {
  ...
  if (ALBUM_ART_CACHE[slug]) {
    track.albumArtURI = ALBUM_ART_CACHE[slug];
    return cb(null, track);
  }
  ...
  lfm.track.getInfo({
    ...
    ALBUM_ART_CACHE[slug] = track.albumArtURI;
    cb(null, track);
  });
};
